...
function [sc_th0, sc_thf, sc_Kmax, lambda] = scaleToStandard(x0, y0, th0, xf, yf, thf, Kmax)
  ...
  sc_Kmax = Kmax * lambda;
  fprintf("INPUT: %f %f %f %f %f %f %f\n", x0, y0, th0, xf, yf, thf, Kmax);
  fprintf("OUTPUT: %f %f %f %f\n", sc_th0, sc_thf, sc_Kmax, lambda);
end
...
function [pidx, curve] = dubins_shortest_path(x0, y0, th0, xf, yf, thf, Kmax)
  ...
  for i = 1:numel(primitives)
    ...
    if (ok && Lcur<L)
      fprintf("LENGTHS[%d]: %f %f %f\n", i, sc_s1_c, sc_s2_c, sc_s3_c);
      L = Lcur;
      sc_s1 = sc_s1_c;
      sc_s2 = sc_s2_c;
      sc_s3 = sc_s3_c;
  ...
  if pidx > 0
    ...
    fprintf("INPUT: %f %f %f\n", x0, y0, th0);
    fprintf("OUTPUT: %f %f %f\n", s1, s2, s3);
    fprintf("CURVATURES: %f %f %f\n", ksigns(pidx,1)*Kmax, ksigns(pidx,2)*Kmax, ksigns(pidx,3)*Kmax);
